...
var _hecele = function (_word) {
    ...
    var spellCount = token.split("").filter(function (c) { return c == "1"; }).length;
...
var hecele = function (word) {
    if (Array.isArray(word)) {
        return word.map(function (w) { return _hecele(w); });
    }
    else if (word.includes(" ")) {
        return word.split(" ").map(function (w) { return _hecele(w); });
    }
    else {
        return _hecele(word);
    }
